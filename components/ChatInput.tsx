...
/** Ikon custom per level reasoning (pengganti emoji di REASONING_META). */
const LEVEL_ICONS = {
  fast: ZapIcon,
  medium: ActivityIcon,
  high: LayersIcon,
...
export default function ChatInput({
  ...
  // Autosize textarea 1–5 baris (maks 132px).
  useEffect(() => {
  ...
  return (
    <div className="sticky bottom-0 shrink-0 border-t border-[var(--border)] bg-[var(--panel)]">
      <div className="mx-auto w-full max-w-[760px] px-3 py-2.5 sm:px-3.5 sm:py-3">
        ...
        {/* Baris model: tombol dropdown + refresh */}
        <div className="flex items-center gap-1.5 py-1.5">
          <div ref={dropRef} className="relative min-w-0 flex-1">
            ...
            >
              ...
              <span className="flex shrink-0 items-center gap-1.5">
                {currentVision && (
                  <span
                    className="inline-flex items-center gap-1 text-[10.5px] font-semibold text-[var(--muted)]"
                    title="Support gambar"
                  >
                    <ImageIcon size={12} />
                    vision
                  </span>
                  ...
                  {filtered.slice(0, 200).map((m) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {m.vision && (
                          <span className="shrink-0" title="Support gambar">
                            <ImageIcon size={14} />
                          </span>
                        )}
                        {m.reasoning && (
                          <span className="shrink-0" title="Support reasoning">
                            <CpuIcon size={14} />
                          </span>
                        )}
                        {selected && (
          ...
          <button
            ...
            className="inline-flex shrink-0 cursor-pointer items-center gap-1 rounded-lg border border-[var(--border)] bg-transparent px-2 py-1 text-xs text-[var(--muted)] transition-colors hover:border-[var(--accent)] hover:text-[var(--ink)] disabled:opacity-50"
          >
            <span className={modelsLoading ? "inline-block animate-spin" : ""}>
              <RefreshIcon size={14} />
            </span>
            <span className="hidden sm:inline">{models.length}</span>
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 px-0.5 pb-2 text-xs text-[var(--muted)]" role="group" aria-label="Level reasoning">
          {LEVELS.map((lv) => {
            ...
            return (
              <button
                ...
                aria-pressed={active}
                className={`inline-flex cursor-pointer items-center gap-1 rounded-full border px-3 py-[5px] text-[11.5px] font-semibold transition-colors ${
                  active
          ...
          {!visionOk && (
            <span className="inline-flex items-center gap-1 text-[11px] text-yellow-500">
              <AlertIcon size={13} />
              Model ini kemungkinan tidak support gambar.
            </span>
          )}
        </div>
            ...
            {images.map((img) => (
              <span key={img.id} className="flex items-center gap-1.5 rounded-[10px] border border-[var(--border)] bg-[var(--surface2)] py-1 pl-1 pr-1.5 text-[11.5px] text-[var(--mid)]">
                ...
                <button
                  ...
                  className="cursor-pointer rounded-md p-0.5 text-[var(--muted)] hover:text-red-400"
                  title="Hapus gambar"
        ...
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            disabled={streaming}
            className="shrink-0 cursor-pointer rounded-xl border border-[var(--border)] bg-transparent p-2.5 text-[var(--muted)] transition-colors hover:border-[var(--accent)] hover:text-[var(--ink)] disabled:opacity-30"
          ...
          {streaming ? (
            ...
            <button
              ...
              className="shrink-0 cursor-pointer rounded-xl bg-[var(--accent)] p-2.5 text-[var(--accent-ink)] transition hover:brightness-[1.07] disabled:cursor-not-allowed disabled:opacity-35"
              title="Kirim"
